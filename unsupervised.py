...
# psl start
parser.add_argument("--no_eval", type=int, default=0, help="Whether eval or not (hard to get ground-truth "
                                                                     "bilingual dico for low-resource langs)")
# psl end
...
trainer = Trainer(src_emb, tgt_emb, mapping, discriminator, params)  # input mapping: 300*300 对角线为1其余为0的矩阵
...
if params.n_refinement > 0:
    ...
    for n_iter in range(params.n_refinement):
        ...
        # apply the Procrustes solution  # 一种归一化处理，通过不断迭代寻找标准形状
        trainer.procrustes()
