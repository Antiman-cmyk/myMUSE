...
class Translator(object):
    ...
    def word_translation(self, src_word):
        if src_word not in self.src_dico.word2id:  # source unknown word
            ...
            for i in range(len(topk_tgt_ids)):
                tgt_word = self.tgt_dico.id2word[topk_tgt_ids.cpu().numpy()[i]]
            ...
            if lm:
                ...
                for sequence, sequence_score in beam:
                    ...
                    for tgt_word, score in topk_zipped:
                        ...
                        lm_score = self.params.lm_scaling * (
    ...
    def corpus_translation(self, corpus, lm):
        ...
        with tqdm(total=total) as pbar:
            for sent in corpus:
                cnt = cnt + 1
                # logger.info("Translating the {}/{} sentence".format(cnt, total))
                trs = self.sent_translation(sent, lm)
                translations.append(trs)
                if cnt % 10 == 0:
                    # logger.info("Translating the {}/{} sentence".format(cnt, total))
                    pbar.set_description("Done the {}/{} sentences".format(cnt, total))
                    pbar.update(10)  # 下一个INFO会紧接上一个pbar
                    # pbar.write('\n')
        pbar.close()
        ...
        logger.info('Writing translations to %s ...' % output_path)  # psl
