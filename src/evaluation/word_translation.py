...
def get_word_translation_accuracy(lang1, word2id1, emb1, lang2, word2id2, emb2, method, dico_eval):
    ...
    if dico_eval == 'default':
        ...
        path = dico_eval
    dico = load_dictionary(path, word2id1, word2id2)  # TODO: how to use dico and emb
    ...
    # nearest neighbors
    if method == 'nn':
        query = emb1[dico[:, 0]]  # psl: words1 in dico -> emb1[words1] -> query num_word*300
        scores = query.mm(emb2.transpose(0, 1))  # psl: emb2.transpose(0, 1) -> 300*200000 -> num_word*200000

    # inverted softmax
    # hubs are words which appear as the nearest neighbour target word to many source words
    # softmax find y closest to Wx given x
    # inverted softmax find x (Wx) given y
    elif method.startswith('invsm_beta_'):
        beta = float(method[len('invsm_beta_'):])
        bs = 128
        word_scores = []
        for i in range(0, emb2.size(0), bs):
            scores = emb1.mm(emb2[i:i + bs].transpose(0, 1))  # psl: (emb1)num_word*300 * (emb2)300*128 -> num_word*128
            scores.mul_(beta).exp_()
            scores.div_(scores.sum(0, keepdim=True).expand_as(scores))  # psl: normalize
