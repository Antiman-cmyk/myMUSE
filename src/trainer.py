...
class Trainer(object):
    ...
    def __init__(self, src_emb, tgt_emb, mapping, discriminator, params):
        ...
        # optimizers
        if hasattr(params, 'map_optimizer'):  # default sgd lr:0.1
            optim_fn, optim_params = get_optimizer(params.map_optimizer)
            self.map_optimizer = optim_fn(mapping.parameters(), **optim_params)
        if hasattr(params, 'dis_optimizer'):  # default sgd lr:0.1
    ...
    def dis_step(self, stats):
        ...
        """
        self.discriminator.train()  # set training mode
    ...
    # psl start
    # unaccessible low-resource gt dico for eval
    # ignore the best mapping but save the last one
    def save_last(self):
        """
        Save the last model for the given validation metric.
        """
        W = self.mapping.weight.data.cpu().numpy()
        path = os.path.join(self.params.exp_path, 'last_mapping.pth')
        logger.info('* Saving the last mapping to %s ...' % path)
        torch.save(W, path)

    def reload_last(self):
        path = os.path.join(self.params.exp_path, 'last_mapping.pth')
        logger.info('* Reloading the last model from %s ...' % path)
        # reload the model
